#!/usr/bin/env groovy
/*
 * Fleet Control Config Schema Generator — Java Agent
 *
 ...
 *   2. <repo-root>/.fleetControl/schemaGeneration/newrelic.yml
...
final File DEFAULT_YML_PATH = new File(SCRIPT_DIR, 'newrelic.yml')
...
//
// Why old leaves don't survive a type change: if the YAML now declares
// `enabled: 5` where it used to be `enabled: true`, the schema must reflect
// the current shape so validation matches reality. Only the *key* is
// preserved across runs, not the constraint that no longer applies.
// ---------------------------------------------------------------------------
static Map<String, Object> mergeSchemas(Map<String, Object> oldS, Map<String, Object> newS) {
    if (oldS == null || oldS.isEmpty()) return newS

    Map<String, Object> merged = new LinkedHashMap<String, Object>(newS)
    ...
    if (!oldProps.isEmpty() || !newProps.isEmpty()) {
        merged.put('properties', mergeProperties(oldProps, newProps))
    }

    return merged
}

static Map<String, Object> mergeProperties(Map<String, Object> oldProps, Map<String, Object> newProps) {
    Map<String, Object> result = new LinkedHashMap<String, Object>()

    // Walk new first so keys present in both retain the new schema's ordering.
    newProps.each { String key, Object newVal ->
        if (oldProps.containsKey(key)
                && newVal instanceof Map
                && oldProps.get(key) instanceof Map
                && ((Map) newVal).get('type') == 'object'
                && ((Map) oldProps.get(key)).get('type') == 'object') {
            result.put(key, mergeSchemas((Map<String, Object>) oldProps.get(key),
                                         (Map<String, Object>) newVal))
        } else {
            result.put(key, newVal)
        }
    }
    // Carry forward keys that only exist in the old schema.
    oldProps.each { String key, Object oldVal ->
...
static Map<String, Object> generateSchema(String rawText, Set<String> excludeKeys,
    ...
    return [
        ...
        description:          'Fleet Control configuration schema for the New Relic Java agent. ' +
                              'Generated from .fleetControl/schemaGeneration/newrelic.yml.',
        type:                 'object',
...
if (binding.variables.get('TEST_MODE') != true) {
    ...
    Map<String, Object> generated = generateSchema(rawText, EXCLUDE_KEYS, ENUM_OVERRIDES, TYPE_OVERRIDES)

    Map<String, Object> oldSchema = loadExisting(SCHEMA_PATH)
    Map<String, Object> newSchema = mergeSchemas(oldSchema, generated)
