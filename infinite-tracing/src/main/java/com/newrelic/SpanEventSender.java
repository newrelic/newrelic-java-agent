...
class SpanEventSender implements Runnable {
    ...
    private boolean awaitReadyObserver(ClientCallStreamObserver<V1.Span> observer) {
        if (observer.isReady()) {
            return true;
        }
        try {
    ...
    private static V1.Span convert(SpanEvent spanEvent) {
    ...
    private void writeToObserver(ClientCallStreamObserver<V1.Span> observer, V1.Span span) {
        try {
