...
import com.agent.instrumentation.akka.http102.{PathMatcherScalaUtils, PathMatcherUtils}
import com.newrelic.api.agent.weaver.{MatchType, Weave, Weaver}

@Weave(`type` = MatchType.BaseClass, originalName = "akka.http.scaladsl.server.PathMatcher")
...
@Weave(`type` = MatchType.ExactClass, originalName = "akka.http.scaladsl.server.PathMatcher$")
