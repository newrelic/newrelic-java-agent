...
class PlayServer() {
  ...
  def start(port: Int, async: Boolean) = {
    ...
    if (async) {
      ...
      bindingFuture = Http().newServerAt(interface = "localhost", port).bind(asyncRequestHandler)
      ...
      bindingFuture = Http().newServerAt(interface = "localhost", port).bindSync(requestHandler)
    }
