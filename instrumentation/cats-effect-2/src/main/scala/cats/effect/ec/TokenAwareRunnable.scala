...
final class TokenAwareRunnable(val delegate: Runnable) extends Runnable {
  ...
  logTokenInfo(tokenAndRefCount, "token field info set")
  clearTxn()
  ...
  def clearTxn(): Unit = {
    ...
    if(txn != null) {
      ...
      if (AgentBridge.getAgent.getLogger.isLoggable(Level.FINEST)) {
        AgentBridge.getAgent.getLogger.log(Level.FINEST, s"Txn ${txn.hashCode()} cleared from Thread: ${Thread
  ...
  def logTokenInfo(tokenAndRefCount: AgentBridge.TokenAndRefCount, msg: String): Unit = {
    if (AgentBridge.getAgent.getLogger.isLoggable(Level.FINEST)) {
      ...
      AgentBridge.getAgent.getLogger.log(Level.FINEST,
        s"[${Thread.currentThread().getName}] ${tokenMsg}: [EC TokenAwareRunnable] token info ${msg}")
    }
  }
