...
private final class IOFiber_Instrumentation[A](private[this] val initMask: Int,
  ...
  private val tokenAndRefCount: AtomicReference[AgentBridge.TokenAndRefCount] =
    new AtomicReference(getThreadTokenAndRefCount)

  incrementTokenRefCount(this.tokenAndRefCount.get())
  logTokenInfo(tokenAndRefCount.get(), "IOFiber token info set")
  ...
  private def done(oc: OutcomeIO[A]): Unit = {
    ...
    if (tokenAndRefCount != null) {
      ...
      attemptExpireTokenRefCount(tokenAndRefCount.get())
    }
  }
