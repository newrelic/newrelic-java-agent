...
import com.nr.agent.instrumentation.httpclient30.InboundWrapper;
import com.nr.agent.instrumentation.httpclient30.OutboundWrapper;
        ...
        if (!checkForIgnoredSocketCall(method)) {
            try {
                ...
                AgentBridge.getAgent().getLogger().log(Level.FINER, e,
                        "Unable to process inbound response headers for execute()");
            }
        }
