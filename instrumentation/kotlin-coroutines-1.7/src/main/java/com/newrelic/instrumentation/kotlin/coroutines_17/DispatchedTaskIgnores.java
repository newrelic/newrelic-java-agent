...
public class DispatchedTaskIgnores {
	...
	public static void addIgnoredTasks(Collection<String> toIgnore, Collection<String> toIgnoreRegEx) {
		...
		for(String ignoredTask : ignoredTasks) {
			NewRelic.getAgent().getLogger().log(Level.FINER, "Ignoring dispatched task: {0}", ignoredTask);
		}
		for(String task : toIgnoreRegEx) {
			ignoredTasksPatterns.add(Pattern.compile(task));
			NewRelic.getAgent().getLogger().log(Level.FINER, "Ignoring dispatched tasks matching regex : {0}", task);
		}
	}
