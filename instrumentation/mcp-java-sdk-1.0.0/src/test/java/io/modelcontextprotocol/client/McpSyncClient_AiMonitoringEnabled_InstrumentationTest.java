...
public class McpSyncClient_AiMonitoringEnabled_InstrumentationTest {
    ...
    private void callToolInTransaction() {
        try {
            mockMcpSyncClient.callTool(new McpSchema.CallToolRequest("testTool", null));
        } catch (Exception ignored) {}
    }
    ...
    private void readResourceInTransaction() {
        try {
            mockMcpSyncClient.readResource(new McpSchema.ReadResourceRequest("https://example.com/resource"));
        } catch (Exception ignored) {}
    }
    ...
    private void getPromptInTransaction() {
        try {
            mockMcpSyncClient.getPrompt(new McpSchema.GetPromptRequest("testPrompt", null));
        }  catch (Exception ignored) {}
    }
}