...
public class NRAsyncHandler<T> {
    ...
    public AsyncHandler.STATE onHeadersReceived(HttpResponseHeaders headers) {
        if (!headers.isTraillingHeadersReceived()) {
            if (segment != null) {
                HashMap<String, List<String>> nrHeaders = new HashMap<>();
                Headers responseHeaders = headers.getHeaders();
                if (responseHeaders != null) {
                    for (Map.Entry<String, List<String>> header : responseHeaders) {
                        nrHeaders.put(header.getKey(), header.getValue());
                    }
                }
                inboundHeaders = new InboundWrapper(nrHeaders);
            }
