...
public class ServletHelper {
    ...
    private static boolean setTxNameUsingServletName(ServletConfig servletConfig, Servlet servlet) {
        ...
        AgentBridge.getAgent().getLogger().log(Level.FINEST, "Servlet24/ServletHelper::setTxNameUsingServletName: calling transaction.setTransactionName with " +
                        "priority: SERVLET_NAME and override false, category: Servlet, txn {0}, ",
                AgentBridge.getAgent().getTransaction().toString());
        NewRelic.getAgent().getLogger().log(Level.FINER, "Servlet24/ServletHelper::setTxNameUsingServletName: Setting transaction name using servlet name: {0}", txName);
