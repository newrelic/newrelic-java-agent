# Spring AI Client Chat Instrumentation
...
An `llm: true` agent attribute will be set on all Transaction events where one of the supported SpringAI `ChatClient` methods is invoked within an active
transaction.
...
custom attributes added by the `addCustomParameters` API to be added to `LlmEvent`s the API calls must occur before invoking the SpringAI `ChatClient`.
