...
class SpringTestHandler {
    static HttpHandler httpHandler(URI endPoint) {
        ...
        RouterFunction<ServerResponse> route = route(GET("/"), SpringTestHandler::sample)
                .andRoute(GET("/helloWorld"), SpringTestHandler::helloWorld)
                .andNest(GET("/language/{language}"), route(GET("/nested"), SpringTestHandler::nested))
                .andRoute(POST("/createUser").and(contentType(APPLICATION_JSON)), SpringTestHandler::createUser)
                .andRoute(POST("/path/{where}"), SpringTestHandler::pathRequest)
                .andRoute(queryParam("bar", str -> str.endsWith("java")), SpringTestHandler::queryParameter)
                .andRoute(headers(SpringTestHandler::headerPredicate), SpringTestHandler::headersHandler)
                .andRoute(contentType(MediaType.APPLICATION_PDF, MediaType.APPLICATION_XML).and(POST("/uploadPDF")), SpringTestHandler::pdfHandler)
                .andRoute(POST("/person").and(accept(APPLICATION_JSON)).and(contentType(APPLICATION_JSON)), SpringTestHandler::createUser)
                .andRoute(path("/path-to-greatness").and(accept(MediaType.TEXT_PLAIN)), SpringTestHandler::pathToGreatness)
                .andRoute(pathExtension("html"), SpringTestHandler::htmlHandler)
    ...
    static HttpHandler httpHandlerNested() {
        ...
        RouterFunction<ServerResponse> route = nest(path("/person"),
                ...
                .andRoute(path("/bad-status-code"), SpringTestHandler::badStatusCode);
