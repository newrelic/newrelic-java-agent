...
import com.nr.agent.instrumentation.sttp.DelegateFutureAkkaStreams
...
class AkkaHttpBackend_Instrumentation {
  def apply(
           ...
           ): SttpBackend[Future, AkkaStreams with WebSockets] = new DelegateFutureAkkaStreams(Weaver.callOriginal())
}
