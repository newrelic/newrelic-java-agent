...
public class RoutingTest extends VertxTestBase{
    ...
    public void testExactPath() {
        ...
        try {
            ...
            getRequest("/swiss-cheese-knife?medium=true&looknomgi=omgno", server, port)
    ...
    public void testRegexBeginsWith() {
        ...
        try {
            ...
            getRequest("/best/first/place/to/understand/your/", server, port)
            ...
            getRequest("/best/first/place/to/understand/your/digital", server, port)
                    .then().body(containsString("BestFirstPlace sent response"));
            ...
            getRequest("/best/first/place/to/understand/your/digital/biscuit", server, port)
                    .then().body(containsString("BestFirstPlace sent response"));
            ...
            getRequest("/best/first/place/to/understand/your/digital/biscuit?nutella=yes", server, port)
                    .then().body(containsString("BestFirstPlace sent response"));
    ...
    public void testCaptureParams() {
        ...
        try {
            ...
            getRequest("/coffee/ghana/medium/", server, port).then().body(containsString("CoffeeHandler sent response"));
            ...
            getRequest("/coffee/gresham/french/?nomgi=wat", server, port).then().body(containsString("CoffeeHandler sent response"));
            attributes = getAttributesForTransaction("OtherTransaction/Vertx/coffee/:origin/:roast/ (GET)");
            assertNotNull(attributes.get("CoffeeHandler"));
            InstrumentationTestRunner.getIntrospector().clear();
        } finally {
    ...
    public void testRouteRegex() {
        ...
        try {
            ...
            getRequest("/newrelic/java/agent", server, port).then().body(containsString("Agent sent response"));
            attributes = getAttributesForTransaction("OtherTransaction/Vertx/.*agent (GET)");
            assertNotNull(attributes.get("Agent"));
            InstrumentationTestRunner.getIntrospector().clear();

            getRequest("/newrelic/agent", server, port).then().body(containsString("Agent sent response"));
            attributes = getAttributesForTransaction("OtherTransaction/Vertx/.*agent (GET)");
            assertNotNull(attributes.get("Agent"));
            InstrumentationTestRunner.getIntrospector().clear();

            getRequest("/agent", server, port).then().body(containsString("Agent sent response"));
            attributes = getAttributesForTransaction("OtherTransaction/Vertx/.*agent (GET)");
            assertNotNull(attributes.get("Agent"));
        } finally {
            vertx.close();
    ...
    public void testRouteRegexCaptureParams() {
        ...
        try {
            ...
            getRequest("/tools/drill123", server, port).then().body(containsString("TwoGroups sent response"));
            attributes = getAttributesForTransaction("OtherTransaction/Vertx/\\/([^\\/]+)\\/([^\\/]+) (GET)");
            assertNotNull(attributes.get("TwoGroups"));
            InstrumentationTestRunner.getIntrospector().clear();

            getRequest("/first/second?query=parameters&favorite=red", server, port).then().body(containsString("TwoGroups sent response"));
            attributes = getAttributesForTransaction("OtherTransaction/Vertx/\\/([^\\/]+)\\/([^\\/]+) (GET)");
            assertNotNull(attributes.get("TwoGroups"));
            InstrumentationTestRunner.getIntrospector().clear();
        } finally {
    ...
    public void testRouteByHttpMethod() {
        ...
        try {
            ...
            getRequest("/products", server, port).then().body(containsString("ProductHandlerGet sent response"));
            attributes = getAttributesForTransaction("OtherTransaction/Vertx/products (GET)");
            assertNotNull(attributes.get("ProductHandlerGet"));
            InstrumentationTestRunner.getIntrospector().clear();

            postRequest("/products/upload", server, port).then().body(containsString("ProductHandlerPost sent response"));
    ...
    public void testRouteByMimeType() {
        ...
        try {
            ...
            getRequest(server, port).then().body(containsString("TextPlainHandler"));
    ...
    public void testRouteCombinedCriteria() {
        ...
        try {
            ...
            postRequest("/myapi/orders", server, port).then().body(containsString("OrdersHandler sent response"));
    ...
    public void testSubRouters() {
        ...
        try {
            ...
            int port = getAvailablePort();
            HttpServer server = createServer(vertx, mainRouter, port);
            getRequest("/api/hello", server, port).then().body(containsString("HelloHandler"));
    ...
    public void testRerouteHandler() {
        ...
        try {
            ...
            getRequest("/some/path", server, port).then().body(CoreMatchers.containsString("Reroute: bar"));
