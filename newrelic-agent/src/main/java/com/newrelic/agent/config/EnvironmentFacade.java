    ...
    static EnvironmentFacade getInstance() {
        ...
        if (agentArgs != null) {
            try {
                ...
                @SuppressWarnings("unchecked")
                final Map<String, String> env = (Map<String, String>) map.get(Discovery.ENVIRONMENT_AGENT_ARGS_KEY);
                if (env == null) {
                    throw new IllegalArgumentException("No environment was provided for attach");
                }
                return new MapEnvironmentFacade(env);
