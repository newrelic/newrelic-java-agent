...
public class ProfileSession {
    ...
    public ProfileSession(ProfilerService profilerService, ProfilerParameters profilerParameters) {
        this.sessionId = TransactionGuidFactory.generate16CharGuid();
    ...
    private IProfile createProfile(ProfilerParameters profilerParameters) {
        return new Profile(profilerParameters, sessionId, ServiceFactory.getThreadService().getThreadNameNormalizer());
    }
    ...
    void start() {
        ...
        if (samplePeriodInMillis == durationInMillis) {
            ...
            getLogger().info(
                    MessageFormat.format("Starting profiling session. Duration: {0} ms, sample period: {1} ms",
                            durationInMillis, samplePeriodInMillis));
    ...
    void transactionFinished(TransactionData transactionData) {
        profile.getTransactionProfileSession().transactionFinished(transactionData);
    }
