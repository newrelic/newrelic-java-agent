...
public class SamplerFactoryTest {
    ...
    public void testGetSamplerForType() {
        //TODO CHANGE OUT THE PLACEHOLDER NAMING
        String FAKE_APP_NAME = "FAKE_APP_NAME";
        ...
        assertEquals(SamplerType.ALWAYS_ON, SamplerFactory.createSampler(FAKE_APP_NAME, mockSamplerConfig).getType());

        when(mockSamplerConfig.getSamplerType()).thenReturn(SamplerConfig.ALWAYS_OFF);
        assertEquals(SamplerType.ALWAYS_OFF, SamplerFactory.createSampler(FAKE_APP_NAME, mockSamplerConfig).getType());

        when(mockSamplerConfig.getSamplerType()).thenReturn("probability"); //this one is not supported in config yet
        assertEquals(SamplerType.PROBABILITY, SamplerFactory.createSampler(FAKE_APP_NAME, mockSamplerConfig).getType());

        when(mockSamplerConfig.getSamplerType()).thenReturn(SamplerConfig.TRACE_ID_RATIO_BASED);
        assertEquals(SamplerType.TRACE_ID_RATIO_BASED, SamplerFactory.createSampler(FAKE_APP_NAME, mockSamplerConfig).getType());

        when(mockSamplerConfig.getSamplerType()).thenReturn(SamplerConfig.ADAPTIVE);
        assertEquals(SamplerType.ADAPTIVE, SamplerFactory.createSampler(FAKE_APP_NAME, mockSamplerConfig).getType());
