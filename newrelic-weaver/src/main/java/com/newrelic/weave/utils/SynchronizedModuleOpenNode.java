...
/**
 * This class exists only to work around the fact that ModuleOpenNode instances are NOT thread safe. If multiple threads
 ...
 *
 * By synchronizing on the {@link #accept(ModuleVisitor)} method we are ensuring that any usage of this ModuleOpenNode
 ...
 *
 * NOTE: If you are upgrading from org.ow2.asm:asm:9.2 / org.ow2.asm:asm-tree:9.2 make sure to double
 * check that this class hasn't changed between versions.
 */
public class SynchronizedModuleOpenNode extends ModuleOpenNode {

    public SynchronizedModuleOpenNode(final String packaze, final int access, final List<String> modules) {
        super(packaze, access, modules);
    }

    /**
     * This method is synchronized in order to prevent issues with bytecode modification that
     * occur when multiple threads call accept on the same SynchronizedModuleOpenNode instance.
     */
