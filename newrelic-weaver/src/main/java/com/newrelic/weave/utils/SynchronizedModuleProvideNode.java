...
/**
 * This class exists only to work around the fact that ModuleProvideNode instances are NOT thread safe. If multiple threads
 ...
 *
 * By synchronizing on the {@link #accept(ModuleVisitor)} method we are ensuring that any usage of this ModuleProvideNode
...
public class SynchronizedModuleProvideNode extends ModuleProvideNode {
    ...
    public SynchronizedModuleProvideNode(final String service, final List<String> providers) {
        super(service, providers);
    }

    /**
     * This method is synchronized in order to prevent issues with bytecode modification that
     * occur when multiple threads call accept on the same SynchronizedModuleProvideNode instance.
     */
